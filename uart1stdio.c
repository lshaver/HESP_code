...
{
    UART1_BASE, UART1_BASE, UART2_BASE
};
...
{
    SYSCTL_PERIPH_UART1, SYSCTL_PERIPH_UART1, SYSCTL_PERIPH_UART2
};
...
//! functions: UART1printf() or UART1gets().  This function assumes that the
...
//*****************************************************************************
void
UART1StdioConfig(uint32_t ui32PortNum, uint32_t ui32Baud, uint32_t ui32SrcClock)
{
    //
    // Check the arguments.
    //
...
//*****************************************************************************
int
UART1write(const char *pcBuf, uint32_t ui32Len)
{
#ifdef UART_BUFFERED
    unsigned int uIdx;
...
//
//*****************************************************************************
// MODIFIED FROM ORIGINAL FORM. Compare to originaly in qs-rgb project. I made some 
// changes to simplify, remove echo, and handle newlines a bit differently.
int
UART1gets(char *pcBuf, uint32_t ui32Len)
{
#ifdef UART_BUFFERED
...
//*****************************************************************************
void
UART1vprintf(const char *pcString, va_list vaArgP)
{
...
UART1printf(const char *pcString, ...)
{
    va_list vaArgP;
		...
		// SEE IF THE OTHER UART IS BUSY FIRST!
...
//! received from the UART but not yet read using UART1gets().
//!
//! \return None.
//
//*****************************************************************************
...
//! UART1printf() that is waiting to be transmitted.  On return, the transmit
        ...
        while(MAP_UARTCharsAvail(g_ui32Base))
        {
            ...
            if(!g_bDisableEcho)
            {
                ...
                if((cChar == '\r') || (cChar == '\n') || (cChar == 0x1b))
                {
                    ...
                    // UART1gets() where the line ends.  We also send an
