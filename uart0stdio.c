...
//! functions: UART0printf() or UART0gets().  This function assumes that the
...
//*****************************************************************************
void
UART0StdioConfig(uint32_t ui32PortNum, uint32_t ui32Baud, uint32_t ui32SrcClock)
{
    //
    // Check the arguments.
    //
...
//*****************************************************************************
int
UART0write(const char *pcBuf, uint32_t ui32Len)
{
#ifdef UART_BUFFERED
    unsigned int uIdx;
...
int
UART0gets(char *pcBuf, uint32_t ui32Len)
{
...
//*****************************************************************************
void
UART0vprintf(const char *pcString, va_list vaArgP)
{
...
//! received from the UART but not yet read using UART0gets().
//!
//! \return None.
//
//*****************************************************************************
...
//! UART0printf() that is waiting to be transmitted.  On return, the transmit
        ...
        while(MAP_UARTCharsAvail(g_ui32Base))
        {
            ...
            if(!g_bDisableEcho)
            {
                ...
                if((cChar == '\r') || (cChar == '\n') || (cChar == 0x1b))
                {
                    ...
                    // UART0gets() where the line ends.  We also send an
